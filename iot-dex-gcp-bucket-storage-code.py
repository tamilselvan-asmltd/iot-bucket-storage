...
def count_files_and_write_to_bucket(service_account_info, request_json, bucket_name):
    try:
        ...
        # file_name = str(request_json['BarCode']) + ".json"
        ...
        return "Service account does not have enough permission"
    except Exception as e:
        bucket_name = request_json['Bucket_Name']
        return f"IncorrectBucketName"
...
def Check_bucketname(service_account_info, request_json, bucket_name):
    try:
        # Authenticate using the service account JSON variable
        client = storage.Client.from_service_account_info(service_account_info)
    ...
    if request.method == 'POST':
        ...
        if request_json:
            ...
            if 'BucketName' in request_json:
                ...
                if not bucket_name:
                    # If bucket name is empty, abort with 400 Bad Request

                    return 'EmptyBucketName'
                # Call Check_bucketname function

                return Check_bucketname(service_account_info, request_json, bucket_name)
            ...
            # Check if the JSON payload contains 'Bucket_Name' and 'BarCode' with BarCode not empty
                ...
                # Extract the bucket_name and barcode from the JSON payload
                bucket_name = request_json['Bucket_Name']
                barcode = request_json['BarCode']
                # Check if the bucket name is empty
                if not bucket_name:
                    ...
                    return 'EmptyBucketName'
                # Call count_files_or_folders function
                return count_files_and_write_to_bucket(service_account_info, request_json, bucket_name)


            else:
                # If 'Bucket_Name' or 'BarCode' is missing or BarCode is empty, abort with 400 Bad Request

                return 'EmptyBucketName'
        else:
            # If the request contains no JSON payload, abort with 400 Bad Request
